...
export function MarkEntry() {
  ...
  // Auto-save timer ref
  const autoSaveTimerRef = useRef<NodeJS.Timeout | null>(null);
  const fetchTeacherRef = useRef<boolean>(false); // Prevent duplicate fetches

  // Get teacher profile from localStorage - optimized
  useEffect(() => {
    if (!user || !isTeacher) {
      setTeacher(null);
      return;
    ...
    if (fetchTeacherRef.current) return;
    fetchTeacherRef.current = true;
      ...
      try {
        ...
        if (response.data.success) {
          ...
          if (teacherData) {
            ...
            // Attempt multiple field names for classIds and subjectIds
            let classIds = teacherData.classIds ||
              teacherData.classes ||
              teacherData.assignedClasses ||
              teacherData.classId ||
              [];

            let subjectIds = teacherData.subjectIds ||
              teacherData.subjects ||
              teacherData.assignedSubjects ||
              teacherData.subjectId ||
              [];

            // If classIds is empty, we'll try to derive from classMasterId later
            if (classIds.length === 0) {
              console.log("🔍 No classIds found, will try to derive from classMasterId later.");
            }
            ...
            setTeacher({
              ...
              classIds: classIds,
              acedemicYear: teacherData.acedemicYear || teacherData.academicYear || ""
            });
          } else {
            console.warn("⚠️ No teacher found for username:", user.username);
            setTeacher(null);
          }
        }
      } catch (error) {
  ...
  // If teacher has no classIds but we have classes loaded, try to derive from classMasterId
  useEffect(() => {
    if (!teacher || !user || !isTeacher || classes.length === 0) return;
    if (teacher.classIds && teacher.classIds.length > 0) return; // Already have classIds

    const teacherId = teacher.id;
    const derivedClassIds = classes
      .filter((c) => c.classMasterId === teacherId)
      .map((c) => c.id);

    if (derivedClassIds.length > 0) {
      console.log("📚 Derived classIds from classMasterId:", derivedClassIds);
      setTeacher((prev) => (prev ? { ...prev, classIds: derivedClassIds } : null));
    }
  }, [teacher, classes, user, isTeacher]);
  ...
  // Filter classes for teacher - optimized with memo
  const allowedClasses = useMemo(() => {
    if (!isTeacher) return classes;
    if (!teacher) return [];
    const allowed = classes.filter((c) => teacher.classIds.includes(c.id));
    console.log("📋 Allowed classes for teacher:", allowed.map(c => c.className));
    return allowed;
  }, [isTeacher, teacher, classes]);

  // Filter subjects for teacher - optimized
  const allowedSubjects = useMemo(() => {
    let subs = subjects;
    if (isTeacher && teacher) {
      subs = subs.filter((s) => teacher.subjectIds.includes(s.id));
  ...
  // Set default class and subject when allowedClasses/allowedSubjects change
  useEffect(() => {
    if (allowedClasses.length > 0 && !classId) {
      setClassId(allowedClasses[0].id);
    } else if (allowedClasses.length === 0 && classId) {
      setClassId(""); // Clear if no classes available
    }
  }, [allowedClasses, classId]);
  ...
  useEffect(() => {
    if (allowedSubjects.length > 0 && !subjectId) {
      setSubjectId(allowedSubjects[0].id);
    } else if (allowedSubjects.length === 0 && subjectId) {
      setSubjectId("");
    }
  }, [allowedSubjects, subjectId]);
